...
const Contact = () => {
  ...
  // Initialize EmailJS
  emailjs.init(import.meta.env.VITE_EMAILJS_PUBLIC_KEY || 'v3Lyp_ICS4VXtd17N');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await emailjs.send(
        import.meta.env.VITE_EMAILJS_SERVICE_ID || 'service_xe2gk5n',
        import.meta.env.VITE_EMAILJS_TEMPLATE_ID || 'template_ojtxh1y',
        {
          from_name: formData.name,
          from_email: formData.email,
          message: formData.message,
          to_name: 'Mumin Habeeb',
        }
      );
            ...
            <Card className="p-6 sm:p-8 bg-card/50 backdrop-blur-sm border-accent/20 hover:border-accent/40 transition-all duration-300">
              ...
              <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                ...
                <Button 
                  ...
                  className="w-full bg-gradient-primary hover:glow-green transition-all duration-300 font-mono text-base sm:text-lg py-4 sm:py-6 group disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isSubmitting ? (
                    <>
