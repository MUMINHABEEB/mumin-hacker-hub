      ...
      <div className="container mx-auto px-4">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2 space-y-6">
                <div className="space-y-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="px-3 py-1 bg-secondary/10 text-secondary rounded-full text-sm font-mono">
                      {experience.duration}
                    </span>
                  </div>
                  <h3 className="text-3xl font-bold font-cyber">{experience.title}</h3>
                  <p className="text-xl text-primary font-mono">{experience.company}</p>
                ...
                <div className="space-y-4">
                  <h4 className="font-cyber text-lg">Key Responsibilities:</h4>
                  <div className="space-y-2">
                    {experience.responsibilities.map((responsibility, index) => (
                      <div key={index} className="flex items-start gap-3">
                        <ArrowRight className="text-primary mt-1 flex-shrink-0" size={16} />
                        <span className="text-muted-foreground font-mono text-sm">{responsibility}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <h4 className="font-cyber text-lg">Skills Developed:</h4>
                  <div className="flex flex-wrap gap-2">
                    {experience.skills.map((skill, index) => (
                      <span 
                        key={index}
                        className="px-3 py-1 bg-accent/10 text-accent rounded-full text-sm font-mono border border-accent/20"
                      >
                        {skill}
                      </span>
                    ))}
              ...
              <div className="space-y-6">
                <Card className="p-6 bg-secondary/5 border-secondary/20">
                  <h4 className="font-cyber text-lg mb-4 flex items-center gap-2">
                  ...
                  <div className="space-y-3">
                    {experience.achievements.map((achievement, index) => (
                      <div key={index} className="flex items-start gap-2">
                        ...
                        <span className="text-sm font-mono text-muted-foreground">{achievement}</span>
                      </div>
                    ))}
                  </div>
                </Card>
            ...
            {careerPath.map((phase, index) => (
              ...
              >
                <div className="space-y-4">
                  ...
                  <div>
                    <h4 className="text-lg font-bold font-cyber mb-2">{phase.title}</h4>
        ...
        <div className="text-center mt-16">
          <Card className="p-8 bg-gradient-glow border-primary/20 max-w-2xl mx-auto animate-slide-up">
            <h3 className="text-2xl font-bold font-cyber mb-4">Ready for New Challenges</h3>
