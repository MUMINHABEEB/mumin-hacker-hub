  ...
  useEffect(() => {
    const loadPosts = async () => {
      try {
        ...
        // Fetch all published social media posts from the API
        let posts;
        try {
          posts = await apiHelpers.getSocialMediaPosts();
        } catch (err) {
          console.error('API error, using fallback data:', err);
          posts = fallbackPosts as unknown as typeof posts;
        }

        const postsData = (posts && posts.length > 0) ? posts : fallbackPosts as unknown as typeof posts;
